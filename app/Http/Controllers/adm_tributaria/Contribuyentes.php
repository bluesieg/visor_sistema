...
//        $user = DB::table('adm_tri.contribuyentes')->select('usuario')->where('id_pers', '=', $request['id'])->get();
    ...
    function get_predios_contrib($id) {
        return DB::select("select id_pred_anio,id_pred,sec,mzna,lote,nom_via,nro_mun,mzna_dist,lote_dist,habilitacion,base_impon from adm_tri.vw_predi_urba where id_contrib=$id and anio='".date("Y")."' and pred_anio_activo=1 and pred_contrib_activo=1");
