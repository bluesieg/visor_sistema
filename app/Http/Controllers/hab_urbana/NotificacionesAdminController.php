...
use App\Models\hab_urbana\NotificacionesAdmin;
...
class NotificacionesAdminController extends Controller
{
    ...
    public function create(Request $request)
    {
                ...
                $NotificacionesAdmin->id_expediente=$request['id_expediente'];
                $NotificacionesAdmin->txt_notificacion=utf8_encode($request['txt_notificacion']);
                $NotificacionesAdmin->fec_reg=date("d/m/Y");
                $NotificacionesAdmin->save();
                Return  $NotificacionesAdmin->id_notificacion_admin ;
    ...
    {
         $NotificacionesAdmin = new  NotificacionesAdmin;
         $val= $NotificacionesAdmin::where("id_expediente","=",$id )->first();
         if(count($val)>=1)
         {
    ...
    public function rep_notificacion_verif_admin($id)
    {
        $institucion = DB::select('SELECT * FROM maysa.institucion');
        $notificacion=DB::connection('gerencia_catastro')->table('soft_hab_urbana.notificaciones_admin')->where('id_expediente',$id)->get();
