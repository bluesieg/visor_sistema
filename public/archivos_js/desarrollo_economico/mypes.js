function limpiar_mypes(dialogo)
{
...
function nuevo_mypes(id)
{
    $("#foto_mypes").html("");
    limpiar_mypes("dlg_mypes");
    $("#save_button").show();
    $("#mod_button").hide();
    if(id>0)
    {
        
        $.ajax({url: 'mypes/'+id+'?grid=normal',
        type: 'GET',
        success: function(r) 
        {
            $("#dlg_edit_ruc_mype").val(r[0].ruc);
            $("#dlg_edit_ubicacion_mype").val(r[0].ubicacion);
            $("#dlg_edit_representante_mype").val(r[0].representante);
        ...
        error: function(data) {
            ...
            console.log('error');
            console.log(data);  
            MensajeDialogLoadAjaxFinish('dlg_mypes');
        }
        });
        $("#save_button").hide();
        $("#mod_button").show();
 ...
 $.ajax({url: 'traefoto_lote_id/'+id,
    ...
    success: function(r) 
    {
        ...
        if(r!=0)
        {
            ...
            $("#foto_mypes").html(final);
        }
        else
        {
            $("#foto_mypes").html('<center><img src="img/recursos/Home-icon.png" width="85%"/></center>');
        }
...
function save_mype(tipo)
{
   ...
   {
       url='mypes/'+$("#hidden_dlg_lot").val()+'/edit';
   }
    MensajeDialogLoadAjax('dlg_mypes', '.:: Cargando ...');
    $.ajax({url: url,
    type: 'GET',
    data:{id:$("#hidden_dlg_lot").val(),
        ruc:$("#dlg_edit_ruc_mype").val(),
        ubi:$("#dlg_edit_ubicacion_mype").val(),
        rep:$("#dlg_edit_representante_mype").val(),
        tipo_create:"mype"},
    success: function(r) 
    {
        MensajeDialogLoadAjaxFinish('dlg_mypes'); 
   ...
   MensajeDialogLoadAjax('dlg_view_foto', '.:: Cargando ...');
    ...
    success: function(r) 
    {
        ...
        if(r!=0)
        {
            ...
            $("#dlg_img_view_big, #foto_mypes").html(final);
        }
        else
        {
            $("#dlg_img_view, #foto_mypes").html('<center><img src="img/recursos/Home-icon.png" width="85%"/></center>');
        }
...
function crear_mapa_mypes()
{
    $.ajax({url: 'mypes/0?grid=mapa_mypes',
                    type: 'GET',
                    async: false,
                    success: function(r)
                    {
                        ...
                        map.addLayer(lyr_mypes);
                        var extent = lyr_mypes.getSource().getExtent();
                        map.getView().fit(extent, map.getSize());
...
function iniciar_mypes(id)
{
    ...
    crear_dlg("dlg_mypes",1100,"Areas Mypes");
}