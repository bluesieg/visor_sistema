...
function buscar_materia(){
    ...
    fn_actualizar_grilla('table_asesoria_materia','asesoria_legal/0?grid=materia&descripcion='+descripcion);
}
...
function guardar_editar_datos(tipo) {
    ...
    if (tipo == 1) {
        ...
        $.ajax({
            ...
            url: 'asesoria_legal/create?tipo=materia',
            type: 'GET',
            data: {
            ...
            success: function(data) 
            {
                ...
                fn_actualizar_grilla('table_asesoria_materia');
                $("#dlg_nueva_materia").dialog("close");
            },
            error: function(data) {
                mostraralertas("hubo un error, Comunicar al Administrador");
                MensajeDialogLoadAjaxFinish('table_asesoria_materia');
                console.log('error');
                console.log(data);
            }
        });
        ...
        id_materia = $('#table_asesoria_materia').jqGrid ('getGridParam', 'selrow');

        MensajeDialogLoadAjax('dlg_nueva_materia', '.:: Cargando ...');
        $.ajax({
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            url: 'asesoria_legal/'+id_materia+'/edit?tipo=materia',
            type: 'GET',
            data: {
        	descripcion:descripcion
            },
...
function edit_materia()
{
    ...
    if (id_materia) {
        ...
        $.ajax({url: 'asesoria_legal/'+id_materia+'?show=materia',
            type: 'GET',
            success: function(data)
            {          
        ...
        mostraralertasconfoco("No Hay Registros Seleccionados","#table_asesoria_materia");
    }
}
