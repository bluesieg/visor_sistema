        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" style="margin-bottom: -12px">
                            ...
                            <div class="row">
                                ...
                                <section class="col-lg-5"> 
                                    <button onclick="dlg_gen_resol();" type="button" class="btn btn-labeled btn-success">
                                        <span class="btn-label"><i class="fa fa-file-text-o"></i></span>Generar Resolución (REC)
                                    </button>
                                </section>
                            </div>                            
                        </div>                        
            ...
            <div class="well well-sm well-light" style="margin-top:-20px;">                
                <div class="row">
                    <div class="col-xs-12">                        
                        <div class="row">
                            <section id="content_2" class="col col-lg-12">
                                <table id="tabla_resoluciones"></table>
                                <div id="p_tabla_resoluciones"></div>
                            </section>
                        </div>                                                
                    </div>
                </div> 
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        jQuery("#tabla_resoluciones").jqGrid({
            url: 'all_resoluciones',
            datatype: 'json', mtype: 'GET',
            height: 'auto', autowidth: true,
            toolbarfilter: true,
            colNames: ['Nro', 'Fec. Emi', 'Año','N° Documento', 'Contribuyente o Razon Social','REC','texto','Edit.','Notif.'],
            rowNum: 20, sortname: 'id_resol', sortorder: 'desc', viewrecords: true, caption: 'Resoluciones De Apertura', align: "center",
            colModel: [
                {name: 'id_resol', index: 'id_resol',  align: 'center', width: 50},
                {name: 'fch_resol', index: 'fch_resol', align: 'center', width: 80},                
                {name: 'anio_resol', index: 'anio_resol', align: 'center', width: 60},
                {name: 'nro_resol', index: 'nro_resol', align: 'center', width: 100},                
                {name: 'contribuyente', index: 'contribuyente', align: 'left', width: 250},                            
                {name: 'rec', index: 'rec', align: 'center', width: 100},
                {name: 'texto', index: 'texto',hidden: true},
                {name: 'rec', index: 'rec', align: 'center', width: 100},
                {name: 'notificacion', index: 'notificacion', align: 'center', width: 100}
            ],
            pager: '#p_tabla_resoluciones',
            rowList: [13, 20],
            gridComplete: function () {
                    var idarray = jQuery('#tabla_resoluciones').jqGrid('getDataIDs');
                    if (idarray.length > 0) {
                    var firstid = jQuery('#tabla_resoluciones').jqGrid('getDataIDs')[0];
                            $("#tabla_resoluciones").setSelection(firstid);    
                        }
                },
        ...
        $(window).on('resize.jqGrid', function () {
            $("#tabla_resoluciones").jqGrid('setGridWidth', $("#content_2").width());
        });
        ...
        jQuery("#t_doc_recibidos").jqGrid({
            ...
            height: 'auto', autowidth: true,
            toolbarfilter: true,
            colNames: ['id_gen_fis','id_contrib', 'Nro', 'Fecha','Hora', 'Año','N° Documento', 'Contribuyente o Razon Social','estado','verif','Monto S/.'],
